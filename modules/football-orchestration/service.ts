...
export class FootballProductionOrchestrator{
 ...
 private async enrich():Promise<JobResult>{
  ...
  const fixtures=await this.client.from("fixtures").select("id,provider_fixture_id,kickoff_at,status,competition_id,competitions!inner(enabled,provider_id)").eq("is_demo",false).eq("competitions.enabled",true).eq("status","scheduled").not("provider_fixture_id","is",null).gte("kickoff_at",now.toISOString()).lte("kickoff_at",horizon).order("kickoff_at").limit(100);
  ...
  for(const fixture of selected){const hours=(new Date(fixture.kickoff_at).getTime()-now.getTime())/3_600_000;const categories:string[]=["h2h","form","standings"];if(hours<=48)categories.push("injuries","odds");if(hours<=3)categories.push("lineups","statistics");for(const category of categories){const estimate=provider.estimateFixtureRequests?.([category])??1;if(audits.length+estimate>controls.maxEnrichmentRequestsPerRun)break;const before=audits.length;await service.getSnapshot(fixture.id,fixture.provider_fixture_id!,category as "h2h"|"form"|"standings"|"injuries"|"odds"|"lineups"|"statistics","scheduled");if(audits.length>before)refreshed.push({fixtureId:fixture.id,category});}if(audits.length>=controls.maxEnrichmentRequestsPerRun)break}
