...
import type { BusinessSetupOrder, ConsumerSubscription, ManagedPlatformBilling } from "./domain";
...
export class CustomerBillingRepository {
  ...
  async getForUser(userId: string): Promise<CustomerBillingSnapshot> {
    ...
    const [subResult, orderResult] = await Promise.all([
      this.db.from("consumer_subscriptions").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      this.db.from("orders").select("*").eq("user_id", userId).eq("payment_purpose", "business_setup").order("created_at", { ascending: false }),
    ]);
    ...
    for (const row of orderResult.data ?? []) {
      const { data, error } = await this.db.from("managed_platform_billings").select("*").eq("setup_order_id", row.id).maybeSingle();
