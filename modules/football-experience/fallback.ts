...
export async function getDemoFootballExperience(): Promise<FootballExperienceData> {
  ...
  for (const item of matches.slice(0, 4)) { const report = await intelligenceService.getIntelligenceForFixture(item.fixture.id), summaryFixture = fixtures.find((entry) => entry.id === item.fixture.id)!; reports.push({ id: `demo-report-${item.fixture.id}`, fixture: summaryFixture, headline: `${item.fixture.homeTeam.name} vs ${item.fixture.awayTeam.name} intelligence`, excerpt: report.shortAnalysis, confidence: report.prediction.confidence.value, riskLevel: report.prediction.risk, recommendedMarket: report.recommendedMarket.prediction, analysis: null, confidenceExplanation: report.confidenceExplanation, riskExplanation: report.riskIntelligence.explanation, accessLevel: report.prediction.accessLevel ?? "registered", published: true, forwardPrediction: false, provenance: "demonstration" }); }
  const results = resultRows.map(({ prediction,result }) => { const found = fixtures.find((item) => item.id === result.fixtureId); const [homeScore, awayScore] = result.scoreLabel.split("-").map((value) => Number.parseInt(value.trim(), 10)); const base = found ?? fixtures[0]; const resultFixture = { ...base, id: result.fixtureId, homeScore: Number.isFinite(homeScore) ? homeScore : null, awayScore: Number.isFinite(awayScore) ? awayScore : null, status: result.outcome === "pending" ? "scheduled" : "finished" }; return { fixture: resultFixture, date: result.publishedAt, status: resultFixture.status, provenance: "demonstration" as const,prediction:prediction?.market.label??"Published prediction",market:"MATCH_OUTCOME_1X2" as const,settlementStatus:result.outcome==="pending"?"PENDING" as const:"SETTLED" as const,predictionCorrect:result.outcome==="pending"?null:result.outcome==="won" }; });
