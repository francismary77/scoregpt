...
export async function getDemoFootballExperience(): Promise<FootballExperienceData> {
  ...
  for (const item of matches.slice(0, 4)) { const report = await intelligenceService.getIntelligenceForFixture(item.fixture.id), summaryFixture = fixtures.find((entry) => entry.id === item.fixture.id)!; reports.push({ id: `demo-report-${item.fixture.id}`, fixture: summaryFixture, headline: `${item.fixture.homeTeam.name} vs ${item.fixture.awayTeam.name} intelligence`, excerpt: report.shortAnalysis, confidence: report.prediction.confidence.value, riskLevel: report.prediction.risk, recommendedMarket: report.recommendedMarket.prediction, accessLevel: report.prediction.accessLevel ?? "registered", published: true, forwardPrediction: false, provenance: "demonstration" }); }
