...
export default function Home() {
  return <div className="page-shell">
    <SiteHeader />
    <main>
      <div className="container sales-banner"><span><i /> Built for football intelligence businesses</span><p>Want an AI Sports Intelligence Platform like this for your own brand?</p><Link href={brand.salesPageUrl}>Launch your platform <b>→</b></Link></div>
      <section className="hero"><div className="orb orb-one" /><div className="orb orb-two" /><div className="grid-lines" /><div className="container hero-grid">
        <div className="hero-copy"><span className="eyebrow"><i /> AI-powered football analysis</span><h1>Football intelligence.<br/><em>Powered by AI.</em></h1><p>{brand.siteName} combines current football data, statistical analysis and AI reasoning to deliver transparent match intelligence you can understand.</p><div className="hero-actions"><Link className="button" href="/matches">Explore today&apos;s matches <span>→</span></Link><Link className="button button-ghost" href="/register"><span className="play">✦</span> Try 1 free AI analysis</Link></div><div className="trust-line"><span className="avatars">◉ ◉ ◉</span><span><b>Built for clarity</b><small>No guaranteed outcomes. Just better-informed decisions.</small></span></div></div>
      ...
      <section className="section matches-section"><div className="container"><div className="heading-row"><SectionHeading eyebrow="Today's slate" title="Intelligence for today’s matches" copy="A concise view of the fixtures our intelligence engine is currently analysing."/><Link href="/matches" className="text-link">View all matches →</Link></div><div className="cards-grid">{matches.map(m=><MatchCard key={m.id} match={m}/>)}</div></div></section>
      ...
      <section className="business-section"><div className="container business-inner"><div><span className="eyebrow">For football business owners</span><h2>Want your own AI Sports<br/>Intelligence business?</h2><p>Launch a professional platform built around your market and your audience—without starting from zero.</p><Link href="/sales" className="button">View business packages <span>→</span></Link></div><div className="business-points"><span>Your brand.</span><span>Your domain.</span><span>Your subscribers.</span><span>Your pricing.</span><small>Powered by proven {brand.siteName} technology.</small></div></div></section>
