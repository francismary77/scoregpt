...
export const metadata: Metadata = { title: "AI-Powered Football Predictions & Intelligence", description: "Explore transparent AI-powered football predictions and intelligence with clear confidence, risk and reasoning from 9ja Football AI.", alternates:{canonical:getSiteUrl()} };
  ...
  return <div className="page-shell">
    ...
    <main>
      ...
      <section className="hero"><div className="orb orb-one" /><div className="orb orb-two" /><div className="grid-lines" /><div className="pitch-lines" aria-hidden="true"><i/><i/><i/><i/><i/></div><div className="data-network" aria-hidden="true"><i/><i/><i/><i/><i/><i/></div><div className="scan-light" aria-hidden="true"/><div className="container hero-grid">
        <div className="hero-copy"><span className="eyebrow"><i /> 9ja Football AI</span><h1>AI-Powered Football Predictions<br/><em>&amp; Intelligence.</em></h1><p>{brand.siteName} combines current football data, statistical analysis and AI reasoning to deliver smarter match predictions with clear confidence, risk and reasoning.</p><div className="hero-actions"><Link className="button" href="/register">Try Free Prediction <span>→</span></Link><Link className="button button-ghost" href="/matches">Explore today&apos;s matches</Link></div>{brand.businessSalesMarketingEnabled && <Link href="/sales" className="hero-business-link">Launch Your Own AI Football Intelligence Business <span>→</span></Link>}<div className="trust-line"><span className="avatars">◉ ◉ ◉</span><span><b>Built for clarity</b><small>No guaranteed outcomes. Just better-informed decisions.</small></span></div></div>
        <div className="hero-card-wrap"><div className="hero-card-glow"/><article className="analysis-card"><div className="analysis-head"><span><i /> Featured AI intelligence</span><b>DEMO REPORT</b></div><div className="league-row"><span>{featured.competition.name}</span><span>{featured.fixture.displayKickoff}</span></div><div className="fixture"><div><span className="team-badge red">{featured.fixture.homeTeam.shortName[0]}</span><strong>{featured.fixture.homeTeam.name}</strong><small>Home</small></div><span className="versus">VS</span><div><span className="team-badge blue">{featured.fixture.awayTeam.shortName[0]}</span><strong>{featured.fixture.awayTeam.name}</strong><small>Away</small></div></div><div className="analysis-stats"><div><small>AI confidence</small><b className="accent">{featured.prediction.confidence.value}%</b></div><div><small>Risk level</small><b><span className="risk-dot"/> {featured.prediction.risk}</b></div><div><small>Suggested market</small><b>{featured.prediction.market.label}</b></div></div><div className="reasoning"><div><span>✦</span><b>AI reasoning</b></div><p>{featured.shortAnalysis}</p></div><Link href={`/matches/${featured.fixture.id}`} className="analysis-link">View full analysis <span>→</span></Link><p className="demo-note">Demonstration data · Not generated live · Not betting advice</p></article></div>
      </div></section>
      ...
      <MatchTicker matches={football.ticker} />

      <section className="section matches-section" id="today-matches"><div className="container"><div className="heading-row"><SectionHeading eyebrow="Demo match centre" title="Predictions for demonstration matches" copy="A concise view of structured fixtures from the mock intelligence service."/><Link href="/matches" className="text-link">View all matches →</Link></div><div className="cards-grid">{homepageMatches.map(m=><MatchCard key={m.id} match={m}/>)}</div></div></section>
      ...
      <section className="section results-section"><div className="container results-layout"><div><SectionHeading eyebrow="Radical transparency" title="Every published result stays on the record." copy="Trust is built through accountability. We display wins, losses and void selections openly—without deleting inconvenient outcomes."/><Link href="/results" className="button button-ghost">Explore all results <span>→</span></Link></div><div className="results-card"><div className="results-head"><span>Recent demo results</span><small>Public record</small></div>{football.results.map(r=><div className="result-row" key={r.id}><span className={`state ${r.outcome}`}>{r.outcome === "won" ? "✓" : r.outcome === "lost" ? "×" : "–"}</span><div><b>{r.fixtureLabel}</b><small>{r.marketLabel}</small></div><strong>{r.scoreLabel}</strong><em className={r.outcome}>{r.outcome}</em></div>)}<Link href="/results">View performance record →</Link></div></div></section>

      <section className="section why-section"><div className="container"><SectionHeading eyebrow="Why 9ja Football AI" title="Intelligence designed for better judgement" align="center"/><div className="benefit-grid">{benefits.map(([n,t,c])=><article key={n}><span>{n}</span><h3>{t}</h3><p>{c}</p></article>)}</div></div></section>

      <section className="section membership-section"><div className="container membership-layout"><div><SectionHeading eyebrow="Membership" title="Start free. Go deeper when you’re ready." copy="Explore daily match intelligence at no cost, then unlock a fuller view with Premium."/><p className="pricing-note">Final commercial pricing will be announced after the foundation phase.</p></div><div className="plans"><article><span>Free</span><h3>Essential intelligence</h3><p>For exploring how 9ja Football AI works.</p><ul><li>Daily selected match insights</li><li>Basic confidence scores</li><li>Public results access</li></ul><Link href="/register" className="button button-ghost">Get started free</Link></article><article className="featured-plan"><span>Premium <b>COMING SOON</b></span><h3>Complete match intelligence</h3><p>For people who want the full picture.</p><ul><li>All daily match analyses</li><li>Detailed AI reasoning</li><li>Risk and confidence breakdowns</li></ul><Link href="/pricing" className="button">View membership <span>→</span></Link></article></div></div></section>

      {brand.businessSalesMarketingEnabled && <section className="closing-business-cta"><div className="container"><div><span className="eyebrow">Your next move</span><h2>Ready to Build Your Football Prediction Business?</h2><p>See our Starter and Business Editions, including the first 6 months of managed platform service.</p></div><Link href="/sales" className="button">View Business Packages <span>→</span></Link></div></section>}
