...
const configuredName = (id: string) => footballCompetitions.find((item) => item.id === id)?.name;
...
export async function generateMetadata({ params }: { params: Promise<{ competitionId: string }> }): Promise<Metadata> {
  ...
  const name = configuredName(competitionId);
  const competition = data.competitions.find((item) => item.id === competitionId || item.name === name);
  return competition ? { title: competition.name, description: `Fixtures, results and available intelligence for ${competition.name} on 9ja Football AI.`, alternates: { canonical: `${getSiteUrl()}/competitions/${competitionId}` } } : { title: "Competition unavailable" };
}
...
export default async function CompetitionPage({ params }: { params: Promise<{ competitionId: string }> }) {
  ...
  const fixtures = data.fixtures.filter((item) => item.competition.id === competition.id || item.competition.name === competition.name);
