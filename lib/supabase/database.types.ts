...
export type Database = {
  public: {
    Tables: {
      ...
      orders: Table<BaseRow & { order_number:string;user_id:string|null;buyer_name:string;buyer_email:string;buyer_phone:string|null;brand_name:string|null;package_id:string;package_name:string;amount_minor:number;currency:string;payment_method:string;status:string;verified_at:string|null;payment_purpose:"business_setup"|null;product_key:string|null;price_snapshot_id:string|null;standard_amount_minor:number|null;promotion_id:string|null;business_terms_version:string|null;refund_policy_version:string|null;terms_accepted_at:string|null;order_status:"pending_payment"|"payment_failed"|"paid"|"cancelled";onboarding_status:"not_started"|"awaiting_payment"|"awaiting_client_information";paid_at:string|null;checkout_idempotency_key:string|null;fulfillment_count:number }>;
