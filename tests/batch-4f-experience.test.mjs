...
test("30-competition positioning preserves staged rollout reality",()=>{assert.equal(footballCompetitions.length,30);assert.deepEqual(footballCompetitions.filter(item=>item.enabled).map(item=>item.id),["scottish-premiership"]);assert.equal(footballCompetitions.filter(item=>item.providerId===null).length,24)});
