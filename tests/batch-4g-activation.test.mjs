...
test("enabled competition Stage A preflight is narrow, complete, and makes zero provider calls", async () => {
...
test("fresh Stage A cache estimates zero requests", async () => {
  ...
  const setup = workflow(provider(), 30, repository), plan = await setup.workflow.plan("A", { competitionId: "scottish-premiership" });
...
test("Stage A rejects category expansion beyond metadata and teams", async () => {
  const plan = await workflow().workflow.plan("A", { competitionId: "scottish-premiership", categories: ["metadata", "teams", "fixtures"] });
...
test("live execution requires confirmation, enabled provider, credential, and budget", async () => {
  ...
  await assert.rejects(() => normal.workflow.run("A", { dryRun: false, competitionId: "scottish-premiership" }), /CONSUME_PROVIDER_QUOTA/);
  await assert.rejects(() => normal.workflow.run("A", { dryRun: false, competitionId: "scottish-premiership", confirmation: "WRONG" }), /CONSUME_PROVIDER_QUOTA/);
  await assert.rejects(() => workflow(provider({ enabled: false })).workflow.run("A", { dryRun: false, competitionId: "scottish-premiership", confirmation: "CONSUME_PROVIDER_QUOTA" }), /disabled/);
  await assert.rejects(() => workflow(provider({ credentialConfigured: false })).workflow.run("A", { dryRun: false, competitionId: "scottish-premiership", confirmation: "CONSUME_PROVIDER_QUOTA" }), /credential/);
  const exhausted = workflow(provider(), 1); await assert.rejects(() => exhausted.workflow.run("A", { dryRun: false, competitionId: "scottish-premiership", confirmation: "CONSUME_PROVIDER_QUOTA" }), /budget/);
});
...
test("confirmed execution uses only the approved mocked Stage A bundle", async () => {
  ...
  const result = await setup.workflow.run("A", { dryRun: false, competitionId: "scottish-premiership", confirmation: "CONSUME_PROVIDER_QUOTA" });
...
test("malformed, API-error, pagination, and unexpectedly empty envelopes are rejected", async () => {
  ...
  await assert.rejects(() => make({ response: [], paging: { current: 2, total: 1 } }).fetchCompetitionData("39", "2026", ["metadata"]), (error) => error.providerCategory === "malformed-response" && /pagination/i.test(error.safeDiagnostic));
...
test("verification reads persistence and audit state without a provider", async () => {
  ...
  const report = await createFootballVerificationReport(repository, requests, footballCompetitions, "scottish-premiership", "api-football", 30, now());
