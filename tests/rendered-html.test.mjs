...
for (const [path, expected] of [["/", "AI-Powered Football Predictions"], ["/matches", "Match Centre"], ["/results", "Predictions vs Actual Results"], ["/competitions", "30 Top Football Leagues"], ["/competitions/premier-league", "Premier League"], ["/matches/ars-che-demo", "Recommended market"], ["/register", "Create your free account"], ["/login", "Secure member access"], ["/forgot-password", "Reset your password"], ["/reset-password", "Choose a new password"], ["/checkout/platform/launch", "Payments are not currently available"], ["/checkout/platform/business", "Payments are not currently available"], ["/sales", "Founder Launch Offer"], ["/pricing", "Start free"], ["/about", "Building trust"], ["/contact", "How can we help"]]) {
...
test("Batch 3A sales page preserves approved commercial facts and WhatsApp journeys", async () => {
  ...
  for (const value of ["Launch Edition", "Business Edition", "7–14 Working Days", "14–21 Working Days", "₦500,000", "₦350,000", "₦1,000,000", "₦750,000", "₦18,000/month (approx. £10/month) from month 7", "₦24,000/month (approx. £15/month) from month 7", "First 6 months included", "30 Top Football Leagues & Competitions", "+234 810 501 6931", "Have questions before you decide?", "What We Need From You", "How long does it take to launch my platform?"]) assert.match(visibleHtml, new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
...
test("Batch 4F homepage and football routes render repository read-model sections",async()=>{const home=await(await render("/")).text(),matches=await(await render("/matches")).text(),results=await(await render("/results")).text(),competitions=await(await render("/competitions")).text();for(const value of["Built for 30 Top Football Leagues","Upcoming matches","Football intelligence preview","Recent results"])assert.match(home,new RegExp(value,"i"));assert.match(matches,/Upcoming/);assert.match(matches,/Competition/);assert.match(results,/Predictions vs Actual Results/);assert.match(competitions,/rolling out competition by competition/i);assert.doesNotMatch(home,/all 30 (?:are )?live/i)});
